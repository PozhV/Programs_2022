...
void TestFunctionality(
  ...
  const vector<string>& expected
	  ...
	  TotalDuration search("Total search");
...
void TestTime()
{
	const vector<string> words = {"abc", "a", "c", "b", "d", "e", "f", "abcdef", "k", "affafsgvkwjglksegf", "vegees", "ewnmkemfk.efklme", "Ok", "m"};
	string doc;
	for(int i = 0; i < 100; i++)
	{
		int ind = i % words.size();
		doc += words[ind] + " ";
	}
	//cout<<doc<<endl;
	const vector<string> docs(50000, doc);
	const vector<string> queries = { "a", "b", "c", "d", "i", "f", "g", "k", "l", "m"};
